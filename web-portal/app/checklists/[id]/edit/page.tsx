...
const normalizeOptions = (optionsJson?: string) => {
  ...
  try {
    ...
    // Keep raw string if legacy data is not JSON.
  }
  return optionsJson;
};
...
export default function EditChecklistPage() {
  ...
  const loadTemplate = async () => {
    try {
      const response = await api.get<ChecklistTemplateResponse>(`/checklists/templates/${params.id}`);
      ...
      setItems(
        template.items.map((item) => ({
          ...item,
          optionsJson: normalizeOptions(item.optionsJson),
        }))
      );
    } catch (loadError: any) {
      setError(loadError.response?.data?.message || 'Failed to load checklist template');
    } finally {
    ...
    try {
      ...
      const processedItems = validItems.map((item) => {
        if (item.type === 'SELECT' && item.optionsJson) {
          const options = item.optionsJson.split(',').map((opt) => opt.trim()).filter(Boolean);
          return {
            ...item,
      ...
      await api.patch(`/checklists/templates/${params.id}`, payload);
      router.push('/checklists');
    } catch (submitError: any) {
      setError(
        submitError.response?.data?.error || submitError.message || 'Failed to update checklist template'
      );
    } finally {
      setLoading(false);
    }
    ...
    <Layout>
      <div>
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold text-navy-900">Edit Checklist Template</h1>
        </div>
